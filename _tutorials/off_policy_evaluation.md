...
Offline policy evaluation (OPE) is an active area of research in reinforcement learning. The aim, in a contextual bandit setting, is to take bandit data generated by some policy (let's call it the _logging policy_) and evaluate a new _candidate policy_ offline. The use case is clear: before you deploy a policy, you want to estimate its performance, and compare to what's already deployed. This tutorial walks you through how to do evaluation of new policies.

## Introduction & Motivation

In supervised learning settings, the standard approach to offline evaluation is to train on a train set and estimate generalisation performance on a holdout set. In online learning settings, one typically uses progressive validation. In contextual bandit settings, neither is directly possible, because like all reinforcement learning, there is a partial information problem: you never get to see rewards of actions you didn't take. Your only source of information is the bandit data generated by your logging policy, which might make entirely different choices than your candidate policy.
...
If your logging policy produces bandit data in the standard `cb` format, and you already have a candidate policy, you can use the leagacy `--eval` option to perform OPE. Note that your candidate policy doesn't need to be trained using VW.
...
```
In the toy example above, the candidate agreed with the logging policy for the second and third instances, but disagreed on the first instance.
...
The key metric is `average loss`, which corresponds to the OPE estimate (TODO confirm). How it is calculated depends on the estimator; for example, if you wish to use an estimator other than the default `dr`, you may do so using the `--cb_type` option. For IPS, run `vw --cb 2 --eval -d eval.dat --cb_type ips`:
...
The interpretation of OPE is important to get right: if your candidate policy produces an OPE estimate of `3.0` and your logging policy has an average loss of `6.0` (easily calculated by summing the costs in the bandit data, divided by the number of instances), it means that had you deployed the candidate policy, with no exploration, instead of the logging policy _at the time the logging policy was deployed_, you could have expected to see average costs reduce by 50%.
...
Let's say we have the following training data, generated by a logging policy, save in a file named `train.txt`:
...
The sum of costs in this file is 7, and since there are 6 examples, the average cost of the logging policy is `1.1667`.
